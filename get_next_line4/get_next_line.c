...
/*   Updated: 2024/01/14 21:32:48 by sehwjang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_buffer(char **ret_buf, char *prev_buf, int *malloc_size)
{
	int	len;
	int	temp;
	const int	prev_buf_len = ft_strlen(prev_buf);
	
	if (prev_buf == NULL || prev_buf[0] == '\0')
	{
		*ret_buf = (char *)malloc(prev_buf_len + 1);
		if (ret_buf == MALLOC_ERROR)
			return (MALLOC_ERROR);
		paste(*ret_buf, prev_buf, 0, prev_buf_len);
	}
...
char	*get_next_line(int fd)
{
	...
	int				temp;
	char			prev_buf[BUFFER_SIZE + 1];

	temp = init_buffer(&ret_buf, prev_buf, INIT_LINE_SIZE);
	if (temp == MALLOC_ERROR)
		return (NULL);
	else if (temp == RET_BUF_COMPLETE)
		return ((ret_buf));
