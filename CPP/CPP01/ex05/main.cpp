...
/*   Updated: 2024/08/06 20:51:04 by sehwjang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void replaceStringInFile(const std::string& filename, const std::string& s1, const std::string& s2) {
    ...
    while (std::getline(inputFile, line)) {
        ...
        while ((pos = line.find(s1, pos)) != std::string::npos) {
            result.append(line, 0, pos); // Append part before s1
            result.append(s2); // Append s2 instead of s1
            line = line.substr(pos + s1.length()); // Skip past s1 in the line
            pos = 0; // Reset pos to start searching in the new line segment
        }
        result.append(line); // Append any remaining part of the line
